        ...
        for panel in panels:
            try:
                if hasattr(context, "preferences") and context.preferences:
                    ...
                    if addon and hasattr(addon, "preferences") and hasattr(addon.preferences, "category"):
...
def draw_looptools_ui(layout, context):
    lt = getattr(context.window_manager, "looptools", None)
    if not lt:
        box_err = layout.box()
        box_err.label(text="LoopTools 未正确初始化", icon='ERROR')
        return

    is_edit_mode = (context.mode == 'EDIT_MESH')
    if not is_edit_mode:
        box_warn = layout.box()
        box_warn.label(text="请切换到编辑模式 (Edit Mode) 使用 LoopTools", icon='INFO')
    ...
    if lt.display_circle:
        ...
        col_move = box.column(align=True)
        row = col_move.row(align=True)
        row.prop(lt, "circle_lock_x", text="X", icon='LOCKED' if lt.circle_lock_x else 'UNLOCKED')
        row.prop(lt, "circle_lock_y", text="Y", icon='LOCKED' if lt.circle_lock_y else 'UNLOCKED')
        row.prop(lt, "circle_lock_z", text="Z", icon='LOCKED' if lt.circle_lock_z else 'UNLOCKED')
        col_move.prop(lt, "circle_influence")
    ...
    if lt.display_curve:
        ...
        col_move = box.column(align=True)
        row = col_move.row(align=True)
        row.prop(lt, "curve_lock_x", text="X", icon='LOCKED' if lt.curve_lock_x else 'UNLOCKED')
        row.prop(lt, "curve_lock_y", text="Y", icon='LOCKED' if lt.curve_lock_y else 'UNLOCKED')
        row.prop(lt, "curve_lock_z", text="Z", icon='LOCKED' if lt.curve_lock_z else 'UNLOCKED')
        col_move.prop(lt, "curve_influence")
    ...
    if lt.display_flatten:
        ...
        col_move = box.column(align=True)
        row = col_move.row(align=True)
        row.prop(lt, "flatten_lock_x", text="X", icon='LOCKED' if lt.flatten_lock_x else 'UNLOCKED')
        row.prop(lt, "flatten_lock_y", text="Y", icon='LOCKED' if lt.flatten_lock_y else 'UNLOCKED')
        row.prop(lt, "flatten_lock_z", text="Z", icon='LOCKED' if lt.flatten_lock_z else 'UNLOCKED')
        col_move.prop(lt, "flatten_influence")
    ...
    if lt.display_gstretch:
        ...
        col_move = box.column(align=True)
        row = col_move.row(align=True)
        row.prop(lt, "gstretch_lock_x", text="X", icon='LOCKED' if lt.gstretch_lock_x else 'UNLOCKED')
        row.prop(lt, "gstretch_lock_y", text="Y", icon='LOCKED' if lt.gstretch_lock_y else 'UNLOCKED')
        row.prop(lt, "gstretch_lock_z", text="Z", icon='LOCKED' if lt.gstretch_lock_z else 'UNLOCKED')
        col_move.prop(lt, "gstretch_influence")
        if lt.gstretch_use_guide == "Annotation":
            box.operator("remove.annotation", text="Delete Annotation Strokes")
        if lt.gstretch_use_guide == "GPencil":
    ...
    if lt.display_space:
        ...
        col_move = box.column(align=True)
        row = col_move.row(align=True)
        row.prop(lt, "space_lock_x", text="X", icon='LOCKED' if lt.space_lock_x else 'UNLOCKED')
        row.prop(lt, "space_lock_y", text="Y", icon='LOCKED' if lt.space_lock_y else 'UNLOCKED')
        row.prop(lt, "space_lock_z", text="Z", icon='LOCKED' if lt.space_lock_z else 'UNLOCKED')
        col_move.prop(lt, "space_influence")
...
def register():
    for cls in classes:
        try:
            ...
            print(f"LoopTools: Failed to register {cls.__name__}: {e}")

    try:
        bpy.types.VIEW3D_MT_edit_mesh_context_menu.prepend(menu_func)
    except Exception as e:
        print(f"LoopTools: Failed to prepend context menu: {e}")
...
def unregister():
    for cls in reversed(classes):
        try:
            ...
            print(f"LoopTools: Failed to unregister {cls.__name__}: {e}")

    try:
        bpy.types.VIEW3D_MT_edit_mesh_context_menu.remove(menu_func)
    except Exception as e:
        print(f"LoopTools: Failed to remove context menu: {e}")
