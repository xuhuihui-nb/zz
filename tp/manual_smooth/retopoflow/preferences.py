...
class RF_DummyPrefs:
    ...
    expand_mirror = True
    expand_tools = True
    enable_help_hotkey = True
    enable_issue_hotkey = True
    enable_pie_hotkey = False
    setup_automerge = True
    setup_fade_inactive = True
    setup_object_wires = False
    setup_retopo_overlay = True
    setup_selection_mode = True
    setup_snapping = True
    tweaking_distance = 20
    tweaking_move_hovered_mouse = True
    tweaking_move_hovered_keyboard = False
    tweaking_use_native = False
    name_new = 'Retopology'
    name_search = '_High'
    name_replace = '_Low'
    name_suffix = '_Retopology'
    warn_no_sources = True

    def __getattr__(self, name):
        return True


class RF_Prefs(bpy.types.PropertyGroup):
    _fallback_instance = None

    @classmethod
    def get_prefs(cls, context=None):
        if context is None:
            context = bpy.context
        if hasattr(context, "scene") and context.scene and hasattr(context.scene, "retopoflow_prefs"):
            prefs = context.scene.retopoflow_prefs
            if prefs is not None:
                return prefs
        try:
            root_pkg = __name__.split('.')[0]
            if root_pkg in context.preferences.addons:
                addon = context.preferences.addons[root_pkg]
                if hasattr(addon, "preferences") and addon.preferences is not None:
                    if hasattr(addon.preferences, "expand_tools"):
                        return addon.preferences
        except Exception:
            pass
        if cls._fallback_instance is None:
            cls._fallback_instance = RF_DummyPrefs()
