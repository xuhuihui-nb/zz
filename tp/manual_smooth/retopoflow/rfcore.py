...
from .rfpanels import (
    general_panel, mesh_cleanup_panel, masking_panel, mirror_panel,
...
class RFCore:
    ...
    default_RFTool         = RFTool_Relax       # TODO: should be stored and sticky across sessions
...
def is_mouse_in_view3d_window(context, event):
    if not context.area or context.area.type != 'VIEW_3D':
        return False
    mx, my = event.mouse_x, event.mouse_y
    # Exclude overlay UI regions (N-panel, toolbar, header, HUD)
    for rgn in context.area.regions:
        if rgn.type in {'UI', 'TOOLS', 'HEADER', 'HUD', 'NAVIGATION_BAR', 'EXECUTE'}:
            if rgn.x <= mx < rgn.x + rgn.width and rgn.y <= my < rgn.y + rgn.height:
                return False
    # Check 3D Viewport canvas (WINDOW) region
    for rgn in context.area.regions:
        if rgn.type == 'WINDOW':
            if rgn.x <= mx < rgn.x + rgn.width and rgn.y <= my < rgn.y + rgn.height:
                return True
...
class RFCore_Operator(RFRegisterClass, bpy.types.Operator):
    ...
    def modal(self, context, event):
        ...
        if not RFOperator.active_operator() and event.type == 'LEFTMOUSE' and event.value == 'PRESS':
            # Only intercept mouse press if the click is inside the 3D Viewport WINDOW (canvas) region!
            if is_mouse_in_view3d_window(context, event):
                if RFCore.selected_RFTool_idname == 'retopoflow.tweak':
                    try:
                        bpy.ops.retopoflow.tweak('INVOKE_DEFAULT')
                        return {'RUNNING_MODAL'}
                    except Exception as e:
                        print(f"ZZ: Failed to invoke retopoflow.tweak: {e}")
                elif RFCore.selected_RFTool_idname == 'retopoflow.relax':
                    try:
                        bpy.ops.retopoflow.relax('INVOKE_DEFAULT')
                        return {'RUNNING_MODAL'}
                    except Exception as e:
                        print(f"ZZ: Failed to invoke retopoflow.relax: {e}")
