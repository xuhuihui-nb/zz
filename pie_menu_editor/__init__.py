...
for mod in MODULES:
    ...
    importlib.import_module(".%s" % mod, package=__name__)
...
def register():
    ...
    for mod in MODULES:
        m = sys.modules.get("%s.%s" % (__name__, mod))
...
def draw_pie_menu_editor_ui(layout, context):
    try:
        from .addon import get_prefs
        pr = get_prefs(context)
        if pr and hasattr(pr, "draw_prefs"):
            pr.draw_prefs(context, layout)
        else:
            layout.label(text="饼菜单编辑器首选项未初始化", icon='ERROR')
    except Exception as e:
        layout.label(text=f"饼菜单界面加载错误: {e}", icon='ERROR')
