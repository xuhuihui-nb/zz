...
class PMItem(bpy.types.PropertyGroup):
    ...
    def _parse_keymap(km_name, exists=True, splitter=None):
        ...
        keymaps = set()
        if kc:
            for conf_name in ("user", "default", "addon"):
                conf = getattr(kc, conf_name, None)
                if conf and hasattr(conf, "keymaps"):
                    keymaps.update(conf.keymaps.keys())

        if splitter is None:
            splitter = CC.KEYMAP_SPLITTER

        builtin_kms = (
            "Window", "3D View", "3D View Generic", "Object Mode", "Object Non-modal",
            "Mesh", "Sculpt", "Pose", "Curve", "Armature", "Metaball", "Font", "Lattice",
            "Particle", "Grease Pencil", "Paint Sculpt", "Weight Paint", "Vertex Paint",
            "Texture Paint", "Image", "Node Editor", "Sequencer", "Screen", "Screen Editing"
        )
        ...
        for name in km_name.split(splitter):
            ...
            km_exists = (name in keymaps) or (name in builtin_kms)
            if exists:
                if keymaps and not km_exists:
                    continue
            else:
                if km_exists:
                    continue
    ...
    def update_keymap_item(self, context):
        ...
        pr = get_prefs()
        kmis = self.kmis_map.get(self.name, None)
