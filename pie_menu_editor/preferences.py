...
class WM_OT_pm_import(bpy.types.Operator, ImportHelper):
    ...
    def import_json(self, json_data):
        pr = get_prefs()
        pr.import_json(json_data, mode=self.mode, tags=self.tags)
...
class PMEPreferences(bpy.types.PropertyGroup):
    bl_idname = ADDON_ID
    ...
    _is_loading_user_menus = False

    def update_pme_setting_and_save(self, context=None):
        if PMEPreferences._is_loading_user_menus:
            return
        try:
            self.save_user_menus()
        except Exception:
            pass
    ...
    list_size: bpy.props.IntProperty(
        ...
        max=80,
        subtype='PERCENTAGE',
        update=update_pme_setting_and_save,
    )
    ...
    auto_backup: bpy.props.BoolProperty(
        name="Auto Backup", description="Auto backup menus", default=True, update=update_pme_setting_and_save
    )
    expand_item_menu: bpy.props.BoolProperty(
        name="Expand Slot Tools", description="Expand slot tools", update=update_pme_setting_and_save
    )
    ...
    show_keymap_names: bpy.props.BoolProperty(
        name="Keymap Names", default=False, description="Show keymap names", update=update_pme_setting_and_save
    )

    show_tags: bpy.props.BoolProperty(
        name="Tags", default=False, description="Show tags", update=update_pme_setting_and_save
    )
    ...
    num_icons_per_row: bpy.props.IntProperty(
        name="Icons per Row", description="Icons per row", default=30, min=1, max=100, update=update_pme_setting_and_save
    )
    ...
    show_custom_icons: bpy.props.BoolProperty(
        default=False, description="Show custom icons", update=update_pme_setting_and_save
    )
    show_advanced_settings: bpy.props.BoolProperty(
        default=False, description="Advanced settings", update=update_pme_setting_and_save
    )
    ...
    show_list: bpy.props.BoolProperty(default=True, description="Show the list", update=update_pme_setting_and_save)
    ...
    show_pm_title: bpy.props.BoolProperty(
        name="Show Title", description="Show pie menu title", default=True, update=update_pme_setting_and_save
    )
    ...
    def _update_default_popup_mode(self, context=None):
        self.ed('DIALOG').update_default_pmi_data()
        self.save_user_menus()
    ...
    default_popup_mode: bpy.props.EnumProperty(
        ...
        default='PANEL',
        update=_update_default_popup_mode,
    )
    ...
    def save_user_menus(self):
        try:
            data = self.get_export_data()
            if not data or not data.get("menus"):
                return

            # 保存常规设置、列表宽高、工具栏按钮及视图状态
            settings_props = [
                "list_size", "num_list_rows", "interactive_panels", "auto_backup",
                "expand_item_menu", "hold_time", "chord_time", "use_chord_hint",
                "show_names", "show_hotkeys", "show_keymap_names", "show_tags",
                "group_by", "num_icons_per_row", "pie_extra_slot_gap_size",
                "show_custom_icons", "show_advanced_settings", "show_experimental_open_modes",
                "show_list", "show_sidepanel_prefs", "use_filter", "auto_tag_on_add",
                "cache_scripts", "panel_info_visibility", "show_pm_title",
                "restore_mouse_pos", "use_spacer", "default_popup_mode",
                "use_cmd_editor", "toolbar_width", "toolbar_height", "show_error_trace",
                "tree_mode", "save_tree", "maximize_prefs", "mouse_threshold_float",
                "mouse_threshold_int", "mouse_threshold_bool", "mouse_threshold_enum",
                "mouse_dir_mode"
            ]
            st = {}
            for prop in settings_props:
                if hasattr(self, prop):
                    ...
                    if isinstance(val, (set, tuple)):
                        val = list(val)
                    st[prop] = val
            data["pme_settings"] = st

            data_str = json.dumps(data, indent=2, ensure_ascii=False)

            # 1. 保存到硬盘独立数据文件夹 pie_menu_editor_data/user_menus.json
            data_dir = os.path.abspath(os.path.join(ADDON_PATH, os.pardir, ADDON_ID + "_data"))
            if not os.path.exists(data_dir):
                os.makedirs(data_dir)
            user_menus_path = os.path.join(data_dir, "user_menus.json")
            with open(user_menus_path, "w", encoding="utf-8") as f:
                f.write(data_str)

            # 2. 同步保存到 ZZ_Preferences (随 Blender userpref.blend 持久化)
            try:
                pkg_name = __name__.split('.')[0]
                addon_prefs = bpy.context.preferences.addons.get(pkg_name)
                if addon_prefs and hasattr(addon_prefs.preferences, "pme_data_json"):
                    addon_prefs.preferences.pme_data_json = data_str
            except Exception:
                pass
        except Exception as e:
            print(f"PME: Failed to save user menus: {e}")

    def import_json(self, json_data, mode='ADD', tags=""):
        PMEPreferences._is_loading_user_menus = True
        try:
            self._import_json_impl(json_data, mode=mode, tags=tags)
        finally:
            PMEPreferences._is_loading_user_menus = False

    def _import_json_impl(self, json_data, mode='ADD', tags=""):
        if isinstance(json_data, bytes):
            json_data = json_data.decode("utf-8")
        try:
            data = json.loads(json_data)
        except Exception as e:
            print(f"PME: JSON decode error: {e}")
            return

        pr = self
        if isinstance(data, dict) and "pme_settings" in data:
            st = data["pme_settings"]
            for key, val in st.items():
                if hasattr(pr, key):
                    try:
                        curr_val = getattr(pr, key)
                        if isinstance(val, list) and isinstance(curr_val, set):
                            val = set(val)
                        setattr(pr, key, val)
                    except Exception:
                        pass
        ...
        if isinstance(data, list):
            ...
            try:
                version = data.get("version", "1.13.6")
        ...
        if mode == 'RENAME':
            ...
            for name in pm_names:
                if pr.pie_menus.find(name) != -1:
                    new_names[name] = pr.unique_pm_name(name)

        for menu in menus:
            if mode == 'REPLACE':
                if pr.pie_menus.find(menu[0]) != -1:
            ...
            name = new_names.get(menu[0], menu[0])

            pm = pr.pie_menus.add()
            n = len(menu)
            m_mode = menu[4] if n > 4 else 'PMENU'
            pm.mode = m_mode
            if n > 1:
                pm.km_name = menu[1]
            ...
            for item in items:
                ...
                n_item = len(item)
                if n_item >= 4:
                    if mode == 'RENAME' and item[1] == 'MENU' and item[3] in new_names:
        ...
        pms = [pr.pie_menus[menu[0]] for menu in menus if pr.pie_menus.find(menu[0]) != -1]
    ...
    def load_user_menus(self):
        try:
            ...
            # 1. 优先从硬盘文件读取 (pie_menu_editor_data/user_menus.json)
            data_dir = os.path.abspath(os.path.join(ADDON_PATH, os.pardir, ADDON_ID + "_data"))
            user_menus_path = os.path.join(data_dir, "user_menus.json")
            if os.path.isfile(user_menus_path):
                with open(user_menus_path, "r", encoding="utf-8") as f:
                    data_str = f.read()

            # 2. 其次从 ZZ_Preferences 读取 (userpref.blend)
            if not data_str:
                try:
                    pkg_name = __name__.split('.')[0]
                    addon_prefs = bpy.context.preferences.addons.get(pkg_name)
                    if addon_prefs and hasattr(addon_prefs.preferences, "pme_data_json") and addon_prefs.preferences.pme_data_json:
                        data_str = addon_prefs.preferences.pme_data_json
                except Exception:
                    pass

            if data_str:
                self.import_json(data_str, mode='REPLACE')
                print("PME: Successfully restored user pie menus configuration across sessions!")
                return True
        except Exception as e:
            print(f"PME: Failed to load user menus: {e}")
        return False
    ...
    def init_editors(self):
        if not hasattr(self, "editors") or self.editors is None:
            self.editors = {}
        from .ed_pie_menu import Editor as EdPieMenu
        from .ed_menu import Editor as EdMenu
        from .ed_popup import Editor as EdPopup
        from .ed_macro import Editor as EdMacro
        from .ed_modal import Editor as EdModal
        from .ed_panel_group import Editor as EdPanelGroup
        from .ed_hpanel_group import Editor as EdHPanelGroup
        from .ed_property import Editor as EdProperty
        from .ed_stack_key import Editor as EdStackKey
        from .ed_sticky_key import Editor as EdStickyKey

        for ed_cls in (EdPieMenu, EdMenu, EdPopup, EdMacro, EdModal, EdPanelGroup, EdHPanelGroup, EdProperty, EdStackKey, EdStickyKey):
            try:
                ed_cls()
            except Exception:
                pass
    ...
    def ed(self, id):
        if not hasattr(self, "editors") or not self.editors or id not in self.editors:
            self.init_editors()
        return self.editors.get(id)
...
def register():
    if not hasattr(bpy.types.WindowManager, "pme_prefs"):
        bpy.types.WindowManager.pme_prefs = bpy.props.PointerProperty(type=PMEPreferences)
...
def unregister():
    ...
    if hasattr(bpy.types.WindowManager, "pme_prefs"):
        delattr(bpy.types.WindowManager, "pme_prefs")
