...
class SlideEdgeOperator(bpy.types.Operator):
    ...
    prop_plen: FloatProperty(
        name="Slide size",
        description="Define the Sliding size",
        default=0.04,
        step=0.5,        
    )    
    ...
    prop_select: BoolProperty(
        name="Select result",
        description="Select resulting loop",
        default=False,
    )    

    prop_original_select: BoolProperty(
        name="Select original edges",
        description="Select original edges",
        default=False,
    )    
    ...
    def get_flow(self, es, end):
        ...
        for d2 in ds:            
            ...
            if len(sel) > 2:
                return None
            sel = [e1 for e1 in sel if (e1 not in found)]
            if len(sel) > 0:
                e1 = sel[0]
    ...
    def get_3d_cursor(self, context, event):
        ...
        view_point = view3d_utils.region_2d_to_origin_3d(
            region, region3D, mouse_pos)
        world_loc = view3d_utils.region_2d_to_location_3d(
    ...
    def invoke(self, context, event):
        if context.edit_object:
            ...
            gui.txtall = ['Press Esc to exit', 'Left click to commit', 'Hold Shift key for slow sliding']
            self.process(context)            
