...
class Baker(PropertyGroup):
    ...
    def draw(self, context: Context, layout: UILayout):
        """Compact, space-efficient layout for bake map properties and settings."""
        layout.use_property_split = False
        layout.use_property_decorate = False

        col = layout.column(align=True)

        # 1. 贴图专属扩展属性 (如 Invert, Bevel, Range, Distance 等)
        if self.ID not in ('color', 'emissive'):
            self.draw_properties(context, col)
            col.separator(factor=0.3)

        # 2. 烘焙核心参数 (引擎、采样、对比度、后缀名) 紧凑 2 列并排显示
        if context.scene.gd.engine == 'grabdoc':
            # Row 1: 引擎 & 采样
            r1 = col.row(align=True)
            if len(self.SUPPORTED_ENGINES) < 2:
                r1.enabled = False
            sub_e = r1.row(align=True)
            sub_e.label(text="引擎:")
            sub_e.prop(self, 'engine', text="")
            ...
            if self.engine == 'blender_workbench':
                ...
                prop = 'samples_cycles'
            sub_s = r1.row(align=True)
            sub_s.label(text="采样:")
            sub_s.prop(self, prop, text="")

            # Row 2: 对比度 & 后缀名
            r2 = col.row(align=True)
            sub_c = r2.row(align=True)
            sub_c.label(text="对比度:")
            sub_c.prop(self, 'contrast', text="")

            sub_x = r2.row(align=True)
            sub_x.label(text="后缀名:")
            sub_x.prop(self, 'suffix', text="")

            # Row 3: 功能开关 (材质重新导入 & 禁用滤波) 并排显示
            r3 = col.row(align=True)
            r3.prop(self, 'reimport', text="重新导入材质")
            r3.prop(self, 'disable_filtering', text="禁用滤波")
        else:
            r1 = col.row(align=True)
            r1.label(text="后缀名:")
            r1.prop(self, 'suffix', text="")

        # 3. 兼容性与节点需求信息 (紧凑单行)
        info_list = []
        if not self.MARMOSET_COMPATIBLE:
            info_list.append("不支持 Marmoset")
        if self.node_tree and self.REQUIRED_SOCKETS:
            inputs_fmt = ", ".join(self.REQUIRED_SOCKETS)
            info_list.append(f"必需节点: {inputs_fmt}")
        if self.node_tree and self.OPTIONAL_SOCKETS:
            inputs_fmt = ", ".join(self.OPTIONAL_SOCKETS)
            info_list.append(f"支持节点: {inputs_fmt}")

        if info_list:
            col.separator(factor=0.3)
            row_info = col.row(align=True)
            row_info.active = False
            row_info.label(text=" | ".join(info_list), icon='INFO')
    ...
    enabled: BoolProperty(name="Export Enabled", default=False)
