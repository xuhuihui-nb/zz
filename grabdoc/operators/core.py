...
def _init_baker_dependencies():
    """Lazy import wrapper to avoid circular import with __init__."""
    from .. import init_baker_dependencies
...
class GRABDOC_OT_select_active_baker(Operator):
    """Select active baker item to display preview and settings"""
    bl_idname = "grabdoc.select_active_baker"
    bl_label = "Select Active Baker"
    bl_options = {'INTERNAL'}
    ...
    def execute(self, context: Context):
        gd = context.scene.gd
        gd.active_baker_type = self.map_type
        gd.active_baker_index = self.baker_index
        return {'FINISHED'}
...
classes = (
    ...
    GRABDOC_OT_select_active_baker
)
